...
## 3. 휴대용 문고리 (Portable Anti-Contact Tool; PACT)

   * 이 장치는 문을 개폐하거나 엘리베이터 버튼 등을 누를 때와 같이 다양한 생활 접촉 오염을 줄이고자 개발되었으며, 손을 대신하여 사용할 수 있는 작은 기구입니다.
   * 사용법: "Fixed finger" 부분에 검지손가락을 고정하여 사용합니다. "pressing buttion" 은 엘리베이터 버튼 등을 누를 때 이용, "keyring holder" 부분에는 사원증 목걸이나 키링에 걸어서 사용할 수 있도록 휴대성을 높였습니다. "open/close door" 는 문을 개폐할 때 고리에 걸어 사용할 수 있습니다.
...
## 5. 얼굴/눈 가리개 (Face/eye protector)
   ...
   * 이 장치의 프레임에 안면 보호 가능한 투명아크릴지 (OHP 필름) 등을 고정하여, 사용할 수 있습니다.
   * 이 장치는 4공 펀치를 이용하여 손쉽게 구멍을 뚫어 이용할 수 있게 만들어졌습니다. (규격: 67*80*67 mm)
